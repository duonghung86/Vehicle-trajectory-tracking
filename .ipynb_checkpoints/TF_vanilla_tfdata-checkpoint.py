...
''' Change to tf.data '''
batch_size = 32
train_dataset = tf.data.Dataset.from_tensor_slices((X_train, y_train.values)).batch(batch_size)
test_dataset = tf.data.Dataset.from_tensor_slices((X_test, y_test.values)).batch(batch_size),
...
# fit model
start = time.time()
Monitor = model.fit(train_dataset, epochs=10, 
...
print("The RMSE is {0:.3f} and the model was trained within {0:.3f} sec".format(rms,end-start))
